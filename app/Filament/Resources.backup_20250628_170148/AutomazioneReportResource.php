        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('test_invio')
                    ...
                    ->color('warning')
                    ->action(function (\App\Models\AutomazioneReport $record) {
                        try {
                            // Genera PDF reports usando il nuovo metodo
                            $pdfController = app(\App\Http\Controllers\PdfController::class);
                            $response = $pdfController->generateReportsMensiliTuttiUtenti(request()->merge([
                                'anno' => date('Y'),
                                'mese' => date('n')
                            ]));
                            ...
                            if ($data['success'] && !empty($record->email_destinatari)) {
                                // Invia email con allegati PDF reports
                                $mesi = [1 => 'Gennaio', 2 => 'Febbraio', 3 => 'Marzo', 4 => 'Aprile', 5 => 'Maggio', 6 => 'Giugno', 7 => 'Luglio', 8 => 'Agosto', 9 => 'Settembre', 10 => 'Ottobre', 11 => 'Novembre', 12 => 'Dicembre'];
                                $monthName = $mesi[date('n')];
                                $year = date('Y');
                                
                                foreach ($record->email_destinatari as $email) {
                                    \Illuminate\Support\Facades\Mail::to($email)->send(new \App\Mail\ReportsPdfMail($data['files'], $monthName, $year));
                                }
                                
                                \Filament\Notifications\Notification::make()->title('Email Reports Inviate!')
                                    ->body("PDF reports generati e inviati a " . count($record->email_destinatari) . " destinatari")->success()->send();
                            } else if ($data['success']) {
                                \Filament\Notifications\Notification::make()->title('PDF Reports Generati!')
                                    ->body($data['message'] . " - Nessun destinatario configurato")->warning()->send();
                            } else {
                                throw new \Exception('Errore nella generazione PDF reports');
                            }
                        } catch (\Exception $e) {
                            \Filament\Notifications\Notification::make()->title('Errore Reports!')
                                ->body('Errore: ' . $e->getMessage())->danger()->send();
                        }
                    }),
